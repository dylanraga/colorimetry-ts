...
export const TRC_HLG = new ToneResponse<{
	...
	id: 'hlg',
	name: 'HLG',
	eotf: (V, { peakLuminance = PEAK_LUMINANCE, blackLuminance = BLACK_LUMINANCE, gamma = GAMMA } = {}) => {
	...
	invEotf: (L, { peakLuminance = PEAK_LUMINANCE, blackLuminance = BLACK_LUMINANCE, gamma = GAMMA } = {}) => {
		const _gamma = gamma + 0.42 * Math.log10(peakLuminance / 1000);
		const beta = Math.sqrt(3 * (blackLuminance / peakLuminance) ** (1 / _gamma));
