...
export const itp: ColorDifferenceMethod<{
  ...
  const [I1, T1, P1] = colorA.toSpace(spaces.itp).values;
  const [I2, T2, P2] = colorB.toSpace(spaces.itp).values;
...
export const itp_c: ColorDifferenceMethod<{
  ...
  const [, C1] = colorA.toSpace(spaces.itp_lch).values;
  const [, C2] = colorB.toSpace(spaces.itp_lch).values;
...
export const itp_h: ColorDifferenceMethod<{
  ...
  const { scalar = 720 } = options;
  const [, C1, h1] = colorA.toSpace(spaces.itp_lch).values;
  const [, C2, h2] = colorB.toSpace(spaces.itp_lch).values;
